...
export class AccountService {
  ...
  async getAccountInfo(): Promise<AccountInfo> {
    return this.http.request<AccountInfo>("GET", "/api/v1/account", {}, true);
  }

  async generateListenKey(): Promise<ListenKey> {
    return this.http.request<ListenKey>("POST", "/api/v1/listenKey", {}, true);
  }

  async extendListenKey(listenKey: string): Promise<any> {
    const params = listenKey;
    return this.http.request<any>("PUT", "/api/v1/listenKey", { params }, true);
  }
}
